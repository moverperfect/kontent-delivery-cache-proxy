...
export function isAppInsightsEnabled(): boolean { return enabled && Boolean(client); }
...
export function trackDependency(input: Parameters<TelemetryClient["trackDependency"]>[0], redactQueryValues = true): void {
  if (!isAppInsightsEnabled()) return;
  client!.trackDependency({ ...input, data: input.data ? sanitizeUrl(input.data, redactQueryValues) : input.data });
}
