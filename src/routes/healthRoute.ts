...
export async function registerHealthRoutes(
  ...
  app.get("/healthz", async (_request, reply) => {
    ...
    if (
      config.APPINSIGHTS_ENABLE_AVAILABILITY_TRACKING &&
      ...
      trackAvailability({
        ...
        runLocation: "service",
        message: "healthy",
        properties: { endpoint: "/healthz", status: "ready", failedChecks: "" },
      });
    }
    await reply.send({
      status: "ok",
  ...
  app.get("/readyz", async (_request, reply) => {
    ...
    if (
      ...
      const failedChecks = [
        !cacheDirReadable && "cacheDirReadable",
        !cacheDirWritable && "cacheDirWritable",
      ...
      trackAvailability({
        ...
        runLocation: "service",
        message: status,
        properties: { endpoint: "/readyz", status, failedChecks },
      });
    }
